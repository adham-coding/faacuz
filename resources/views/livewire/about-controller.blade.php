<div>
    ...
    <div>
      <div class="relative mt-1">
        ...
        <input wire:model="searchQuery" value="{{ $searchQuery }}" type="search"
          ...
          placeholder="Search for abouts">
      </div>
    </div>

    <div class="text-center text-2xl text-slate-500">
      {{ $actived ? 'Actived' : 'Removed' }} Abouts
    </div>
    ...
    <div class="flex items-center justify-end">
      @if ($actived)
        <x-btn wire:click="openForm" class="bg-emerald-500 text-emerald-50 hover:bg-emerald-600 active:bg-emerald-400">
          ...
          <span class="ml-1">Create about</span>
        </x-btn>
      @endif
      <x-btn wire:click="$toggle('actived')"
        ...
        <span class="ml-2">{{ $actived ? 'Removed' : 'Actived' }} abouts</span>
      </x-btn>
    </div>
  </div>
  ...
  <table class="w-full overflow-hidden rounded-t-xl bg-slate-50 text-slate-700 shadow-lg">
      ...
      <tr>
        ...
        <th class="px-2 py-4">
          <div class="flex items-center pl-2">
            ...
            <span class="ml-1">{{ __('About title') }}</span>
          </div>
        </th>
        <th class="px-2 py-4">
          <div class="flex items-center pl-2">
            ...
            <span class="ml-1">{{ __('About paragraph') }}</span>
          </div>
        </th>
        <th class="px-2 py-4">
          <div class="flex items-center pl-2">
            ...
            <span class="ml-1.5">About image</span>
          </div>
        </th>
        <th class="w-[10%] py-4">
          <div class="flex items-center justify-center">
            ...
            <span class="ml-1">About state</span>
          </div>
        </th>
        <th class="w-[10%] py-4">
          <div class="flex items-center justify-center">
    ...
    <tbody wire:sortable="changeOrder" class="text-lg">
      @foreach ($abouts as $rowNum => $about)
        <tr wire:sortable.item="{{ $about->id }}" wire:key="item-{{ $about->id }}"
          class="border-b">
          <td wire:sortable.handle class="w-[5%] cursor-grab py-3 text-center">
            {{ ++$rowNum + $abouts->perPage() * ($abouts->currentPage() - 1) }}.
          </td>
          <td class="px-4 py-3 font-semibold capitalize">
            {{ substr($about["title_$locale"], 0, 20) . (strlen($about["title_$locale"]) > 33 ? '...' : '') }}
          </td>
          <td class="px-4 py-3 font-semibold">
            {{ substr($about["paragraph_$locale"], 0, 20) . (strlen($about["paragraph_$locale"]) > 33 ? '...' : '') }}
          </td>
          <td class="px-4 py-3">
            <img class="h-20 w-32 rounded-lg border border-slate-300 object-cover p-1"
              src="{{ str_contains($about->image, 'http') ? $about->image : asset($about->image) }}">
          </td>
          <td class="w-[10%] py-3 text-center">
            <button wire:click="changeState({{ $about->id }})"
              class="{{ $about->state ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} rounded px-2.5 py-0.5 font-semibold shadow-md transition-all hover:scale-105 active:translate-y-[0.15rem]"
              title="Change state">
              {{ $about->state ? 'Active' : 'Passive' }}
            </button>
          </td>
          <td class="w-[10%] py-3 text-center">
            <button wire:click="simpleModal({{ $about->id }}, '{{ $actived ? 'remove' : 'delete' }}')"
              ...
              title="{{ $actived ? 'Removing trash' : 'Deleting about' }}">
          ...
          <td class="w-[10%] py-3 text-center">
            <button wire:click="{{ $actived ? "openForm($about->id)" : "simpleModal($about->id, 'rescue')" }}"
              ...
              title="{{ $actived ? 'Editing about' : 'Rescuing about' }}">
    ...
    <div class="w-96 rounded-lg bg-slate-100 p-5 shadow-lg">
        ...
        <div class="mb-5 flex items-start">
          ...
          <span class="w-3/4 text-lg text-stone-700">
            Are you sure you want to {{ $simpleModalType }} <span
              class="{{ $simpleModalType === 'rescue' ? 'text-green-600 drop-shadow-[0_0_1px_rgba(22,163,74,0.6)]' : 'text-red-500 drop-shadow-[0_0_1px_rgba(220,38,38,0.6)]' }} mr-1 tracking-wide">{{ $aboutTitle }}</span>?
          </span>
        </div>
      </div>
      <div class="flex justify-end">
        ...
        <x-btn
          ...
          wire:click="{{ $simpleModalType }}About()" wire:loading.attr="disabled">
          {{ $simpleModalType }} about
        </x-btn>
      </div>
    </div>
  </div>
  ...
  <div
    ...
    <form wire:submit.prevent="formSetter"
      ...
      <div class="text-center text-3xl">
        {{ __(($aboutId ? 'Edit' : 'Create') . ' about') }}
      </div>

      {{-- Title UZ --}}
      <div class="py-3">
        <x-label-error title="About title (UZ)" error="title_uz" />
        <x-textarea wire:model="title_uz" />
      </div>
      {{-- Title RU --}}
      <div class="py-3">
        <x-label-error title="About title (RU)" error="title_ru" />
        <x-textarea wire:model="title_ru" />
      </div>
      {{-- Title EN --}}
      <div class="py-3">
        <x-label-error title="About title (EN)" error="title_en" />
        <x-textarea wire:model="title_en" />
      </div>
      {{-- Paragraph UZ --}}
      <div class="py-3">
        <x-label-error title="About paragraph (UZ)" error="paragraph_uz" />
        <x-textarea wire:model="paragraph_uz" />
      </div>
      {{-- Paragraph RU --}}
      <div class="py-3">
        <x-label-error title="About paragraph (RU)" error="paragraph_ru" />
        <x-textarea wire:model="paragraph_ru" />
      </div>
      {{-- Paragraph EN --}}
      <div class="py-3">
        <x-label-error title="About paragraph (EN)" error="paragraph_en" />
        <x-textarea wire:model="paragraph_en" />
      </div>
      {{-- Image --}}
      <div class="py-3">
        <x-label-error title="About image" error="image" />
